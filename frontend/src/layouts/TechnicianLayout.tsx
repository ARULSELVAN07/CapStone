...
import { LayoutDashboard, Briefcase, Lock, LogOut, Menu, X, Wrench } from 'lucide-react';
...
const TechnicianLayout: React.FC = () => {
  ...
  const handleLogout = () => { logout(); navigate('/technician/login'); };

  const navItems = [
    { to: '/technician', icon: LayoutDashboard, label: 'Dashboard', end: true },
    { to: '/technician/jobs', icon: Briefcase, label: 'My Jobs' },
    { to: '/technician/change-password', icon: Lock, label: 'Change Password' },
  ];
    ...
    <div className="flex h-screen bg-slate-900 overflow-hidden">
        ...
        <div className="flex items-center justify-between h-16 px-6 border-b border-slate-700">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-amber-400 text-xs">Technician Portal</span>
            </div>
          </div>
            ...
            <NavLink key={to} to={to} end={end} onClick={() => setSidebarOpen(false)}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                  isActive ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-700'
                }`
              }
            >
        ...
        <div className="p-4 border-t border-slate-700">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-400">{user?.employeeId || 'Technician'}</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-slate-800 border-b border-slate-700 flex items-center justify-between px-6">
          ...
          <span className="text-sm text-slate-500 hidden sm:block">BMW SpareHub — Technician Portal</span>
        </header>
